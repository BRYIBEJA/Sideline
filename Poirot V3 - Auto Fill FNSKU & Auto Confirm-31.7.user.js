...
// @version      31.7
// @description  Full Poirot V3 automation + FC Research. SSCC by data-section-type. Settings menu with auto-quantity toggle.
...
(function () {
    ...
    function createSettingsMenu() {
        ...
        headerVersion.textContent = 'v31.7';
    ...
    // Create settings menu on Sideline page
    ...
    function fillQuantityFromFC() {
        // Check if auto-quantity is disabled
        ...
        if (!settings.autoQuantity) {
            ...
            return true; // Return true to prevent the default confirm from firing
        }
    ...
    function mainCheck() {
        ...
        if (isVerifyPage()) {
            ...
            if (!quantityHandled) {
                ...
                // Re-read settings live in case user toggled mid-session
                settings = getSettings();
                if (!settings.autoQuantity) {
                    console.log('[Poirot] Auto-quantity DISABLED. Waiting for manual input.');
                    // Still fill the value but don't auto-confirm
                    const quantity = sessionStorage.getItem('poirot_fc_quantity');
                    if (quantity) {
                        const input = getScanInput();
                        if (input) {
    ...
    console.log('[Poirot Auto-Enter] v31.7 loaded — settings menu + auto-quantity toggle.');
})();
